...
namespace StoreDB.Repositories
{
    ...
    {
        private readonly IRepository<ZipCode> _ZipCode;

        public ZipCodeService(IUnitOfWork unitOfWork)
        {
            _ZipCode = new Repository<ZipCode>(unitOfWork);
        }
        ...
        {
            var query = (from c in this._ZipCode.GetAll()
                         where c.IsEnabled
                         select new
                         {
